...
int WINS_Init (void)
{
	...
	if (winsock_initialized == 0)
	{
		...
		if (r)
		{
			Con_Printf (PRINT_SAFE, "Winsock initialization failed.\n");
			return -1;
		}
	}
	...
	if (gethostname (buff, MAXHOSTNAMELEN) == 0)
	{
		...
		if (local == NULL)
		{
			Con_Printf (PRINT_DEVELOPER, "Winsock TCP/IP Initialization timed out.\n");
	...
	{
		Con_Printf (PRINT_NORMAL, "WINS_Init: Unable to open control socket\n");
	...
	Con_Printf (PRINT_NORMAL, "Winsock TCP/IP Initialized\n");
	tcpipAvailable = true;
...
int WINS_Broadcast (int socket, byte *buf, int len)
{
	...
	if (socket != net_broadcastsocket)
	{
		...
		if (ret == -1)
		{
			Con_Printf (PRINT_NORMAL, "Unable to make socket broadcast capable\n");
			return ret;
		}
	}
