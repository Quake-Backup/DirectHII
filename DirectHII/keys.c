...
#define HISTORY_FILE_NAME "data1/h2_history.txt"
...
void Key_Console (int key)
{
	...
	if (key == K_PGUP)
	{
		...
		if (con_backscroll > con_totallines - (vid.height2d >> 3) - 1)
			con_backscroll = con_totallines - (vid.height2d >> 3) - 1;
...
void Key_Unbind_f (void)
{
	...
	if (Cmd_Argc () != 2)
	{
		Con_Printf (PRINT_NORMAL, "unbind <key> : remove commands from a key\n");
		return;
	}
	...
	if (b == -1)
	{
		Con_Printf (PRINT_NORMAL, va ("\"%s\" isn't a valid key\n", Cmd_Argv (1)));
		return;
	}
...
void Key_Bind_f (void)
{
	...
	if (c != 2 && c != 3)
	{
		Con_Printf (PRINT_NORMAL, "bind <key> [command] : attach a command to a key\n");
		return;
	}
	...
	if (c == 2)
	{
		if (keybindings[b])
			...
			Con_Printf (PRINT_NORMAL, va ("\"%s\" is not bound\n", Cmd_Argv (1)));
...
void Key_Event (int key, qboolean down)
{
	...
	if (down)
	{
		...
		if (key >= 200 && !keybindings[key])
			Con_Printf (PRINT_NORMAL, va ("%s is unbound, hit F4 to set.\n", Key_KeynumToString (key)));
	}
...
void Key_ClearStates (void)
{
	...
	for (i = 0; i < 256; i++)
	{
		// send key up events for all keys that are down
		if (keydown[i])
		{
			Key_Event (i, false);
			keydown[i] = false;
