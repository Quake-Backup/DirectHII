...
static void NET_Listen_f (void)
{
	if (Cmd_Argc () != 2)
	{
		Con_Printf (PRINT_NORMAL, va ("\"listen\" is \"%u\"\n", listening ? 1 : 0));
		return;
	}
...
static void MaxPlayers_f (void)
{
	...
	if (Cmd_Argc () != 2)
	{
		Con_Printf (PRINT_NORMAL, va ("\"maxplayers\" is \"%u\"\n", svs.maxclients));
		return;
	}

	if (sv.active)
	{
		Con_Printf (PRINT_NORMAL, va ("maxplayers can not be changed while a server is running.\n"));
		return;
	}
	...
	if (n > svs.maxclientslimit)
	{
		...
		Con_Printf (PRINT_NORMAL, va ("\"maxplayers\" set to \"%u\"\n", n));
	}
...
static void NET_Port_f (void)
{
	...
	if (Cmd_Argc () != 2)
	{
		Con_Printf (PRINT_NORMAL, va ("\"port\" is \"%u\"\n", net_hostport));
		return;
	}
	...
	if (n < 1 || n > 65534)
	{
		Con_Printf (PRINT_NORMAL, va ("Bad value, must be between 1 and 65534\n"));
		return;
	}
...
{
	Con_Printf (PRINT_NORMAL, va ("Server      Connect  Map         Users\n"));
...
{
	if (hostCacheCount)
		Con_Printf (PRINT_NORMAL, "== end list ==\n\n");
	else
		Con_Printf (PRINT_NORMAL, "No Hexen II servers found.\n\n");
}
...
void NET_Slist_f (void)
{
	...
	if (!slistSilent)
	{
		Con_Printf (PRINT_NORMAL, "Looking for Hexen II servers...\n");
		PrintSlistHeader ();
	}
...
qsocket_t *NET_Connect (char *host)
{
	...
	if (host == NULL)
	{
		...
		Con_Printf (PRINT_NORMAL, va ("Connecting to...\n%s @ %s\n\n", hostcache[0].name, host));
	}
...
int	NET_GetMessage (qsocket_t *sock)
{
	...
	if (sock->disconnected)
	{
		Con_Printf (PRINT_NORMAL, "NET_GetMessage: disconnected socket\n");
		return -1;
	}
...
int NET_SendMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	if (sock->disconnected)
	{
		Con_Printf (PRINT_NORMAL, "NET_SendMessage: disconnected socket\n");
		return -1;
	}
...
void NET_Init (void)
{
	...
	{
		s = (qsocket_t *) Hunk_Alloc (GAME_HUNK, sizeof (qsocket_t));
	...
	Cvar_RegisterVariable (&net_allowmultiple, NULL);
	...
	if (*my_ipx_address)
		Con_Printf (PRINT_DEVELOPER, va ("IPX address %s\n", my_ipx_address));

	if (*my_tcpip_address)
		Con_Printf (PRINT_DEVELOPER, va ("TCP/IP address %s\n", my_tcpip_address));
}
...
void		NET_Shutdown (void)
{
	...
	if (vcrFile != -1)
	{
		Con_Printf (PRINT_NORMAL, "Closing vcrfile.\n");
