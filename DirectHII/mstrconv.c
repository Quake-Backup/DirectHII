...
BOOL ConverterInit (LPSTR szInFile)
{
	...
	MidiData = (byte *) FS_LoadFile (MAP_HUNK, (char *) szInFile); // ??? unsure which hunk ???
...
/* RewindConverter ()                                                         */
/*                                                                           */
/*   This little function is an adaptation of the ConverterIni () code which */
...
/* previous call to ConverterIni ().  It will convert data until an error is */
...
int ConvertToBuffer (DWORD dwFlags, LPCONVERTINFO lpciInfo)
{
	...
	for (; ;)
	{
		...
		if ((nChkErr = AddEventToStreamBuffer (&teTemp, lpciInfo)) != CONVERTERR_NOERROR)
		{
			if (nChkErr == CONVERTERR_BUFFERFULL)
			{
...
BOOL RefillTrackBuffer (PINTRACKSTATE ptsTrack)
{
	...
	if (ptsTrack->dwLeftOnDisk)
	{
		...
		{
			Con_Printf (PRINT_NORMAL, "MIDI: Read operation failed prematurely!!\n");
			ptsTrack->dwLeftInBuffer = dwBytesRead;
			return (TRUE);
		}
		else
