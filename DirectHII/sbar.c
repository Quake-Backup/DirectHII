...
int sb_lines2d; // scan lines to draw
...
static double ChainBeginTime = 0;
static float ChainOldPosition = 0;
...
void SB_Init (void)
{
	...
	BarHeight = BarTargetHeight = BarOldHeight = BAR_TOP_HEIGHT;
	BarBeginTime = 0;
}
...
void SB_Draw (void)
{
	...
	// the crosshair belongs to the sbar
	Draw_CrossHair ();

	if (sb_lines2d > 0)
	{
#if 1
		Draw_TileClear (0, vid.height2d - sb_lines2d, vid.width2d, sb_lines2d);
#else
		int tilewidth = (vid.width2d - 320) >> 1;

		Draw_TileClear (0, vid.height2d - sb_lines2d, tilewidth, sb_lines2d);
		Draw_TileClear (tilewidth + 320, vid.height2d - sb_lines2d, tilewidth, sb_lines2d);
#endif
	}

	if (BarHeight < BarTargetHeight)
	{
		BarHeight = BarOldHeight + (((BarTargetHeight - BarOldHeight) * BarSpeed.value) * (realtime - BarBeginTime));
	...
	{
		BarHeight = BarOldHeight - (((BarOldHeight - BarTargetHeight) * BarSpeed.value) * (realtime - BarBeginTime));
	...
	{
		BarOldHeight = BarHeight;
		BarBeginTime = realtime;
	}
...
static qboolean SetChainPosition (float health, float maxHealth)
{
	...
	{
		ChainBeginTime = realtime;
		ChainOldPosition = ChainPosition;
		return false;
	}

	if (ChainPosition < chainTargetPosition)
	{
		ChainPosition += ((chainTargetPosition - ChainPosition) * 5) * scr_frametime;
	...
	{
		ChainPosition -= ((ChainPosition - chainTargetPosition) * 5) * scr_frametime;
...
void Sbar_DeathmatchOverlay (void)
{
	...
	x = 80 + ((vid.width2d - 320) >> 1);
	y = 62;

	for (i = 0; i < l; i++)
	{
		if (y + 10 >= vid.height2d)
			break;
...
void Sbar_SmallDeathmatchOverlay (void)
{
	...
	else if (DMMode.value == 2)
	{
		...
		y = vid.height2d - BAR_TOP_HEIGHT;
	}
	else y = 0; // this is VERY probably wrong, but likely this can't be called if this condition evaluates anyway
...
static void DrawActiveArtifacts (void)
{
	...
	if (flag & ART_TOMEOFPOWER)
	{
		...
		Draw_TransPic (vid.width2d - art_col, 1, Draw_CachePic (tempStr));
		art_col += 50;
	}
...
static void Sbar_DrawString (int x, int y, char *str)
{
	Draw_String (x + ((vid.width2d - 320) >> 1), y + vid.height2d - (int) BarHeight, str);
}
