...
int TimedRandomTable[TIMEDRANDOMTABLE_SIZE];

int CL_TimedRandom (float offset)
{
	// keep random effects framerate-independent
	return TimedRandomTable[(int) (offset + cl.time * 72.0f) & (TIMEDRANDOMTABLE_SIZE - 1)];
}


int CL_RealTimedRandom (float offset)
{
	// keep random effects framerate-independent
	return TimedRandomTable[(int) (offset + realtime * 72.0f) & (TIMEDRANDOMTABLE_SIZE - 1)];
}


void CL_InitTimedRandom (void)
{
	int i;

	for (i = 0; i < TIMEDRANDOMTABLE_SIZE; i++)
		TimedRandomTable[i] = rand ();
}
...
void MSG_WriteFixed16 (sizebuf_t *sb, float f)
{
	MSG_WriteShort (sb, Q_rint ((f * SHRT_MAX) / 1024.0f));
}
...
{
	MSG_WriteShort (sb, (int) ((f * 65536.0f) / 360.0f) & 65535);
}
...
float MSG_ReadFixed16 (void)
{
	return ((float) MSG_ReadShort ()) / 80.0f;
}
...
{
	return MSG_ReadShort () * (360.0 / 65536.0f);
}
...
void SZ_Alloc (sizebuf_t *buf, int startsize)
{
	...
	buf->data = Hunk_Alloc (GAME_HUNK, startsize);
...
void COM_CheckRegistered (void)
{
...
#endif
		Con_Printf (PRINT_NORMAL, "Playing demo version.\n");
	...
	Con_Printf (PRINT_NORMAL, "Playing retail version.\n");
}
...
void COM_Path_f (void)
{
	...
	Con_Printf (PRINT_NORMAL, "Current search path:\n");

	for (s = com_searchpaths; s; s = s->next)
	{
		if (s->pack)
		{
			Con_Printf (PRINT_NORMAL, va ("%s (%i files)\n", s->pack->filename, s->pack->numfiles));
		}
		else
...
============
*/
// this is prototyped in zone.h because common.h doesn't know what a hunktype_t is
...
pack_t *COM_LoadPackFile (char *packfile)
{
	...
	newfiles = Hunk_Alloc (GAME_HUNK, numpackfiles * sizeof (packfile_t));
	...
	pack = Hunk_Alloc (GAME_HUNK, sizeof (pack_t));
	strcpy (pack->filename, packfile);
	pack->handle = packhandle;
	pack->numfiles = numpackfiles;
	pack->files = newfiles;

	Con_Printf (PRINT_NORMAL, va ("Added packfile %s (%i files)\n", packfile, numpackfiles));
	return pack;
}
...
void COM_AddGameDirectory (char *dir)
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		search = Hunk_Alloc (GAME_HUNK, sizeof (searchpath_t));
		search->pack = pak;
