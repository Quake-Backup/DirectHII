...
static void ParseStream (int type)
{
	...
	{
		Con_Printf (PRINT_NORMAL, "stream list overflow\n");
		return;
	}
...
void CL_UpdateTEnts (void)
{
	...
	for (j = 0, stream = cl_Streams; j < MAX_STREAMS; j++, stream++)
	{
		if (!stream->models[0]) // || stream->endTime < cl.time)
		{
			// Inactive
...
static entity_t *NewStreamEntity (void)
{
	...
	if (StreamEntityCount == MAX_STREAM_ENTITIES) return NULL;
