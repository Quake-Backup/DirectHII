...
void D_ShaderOnLost (void)
{
}

void D_ShaderOnReset (void)
{
}

void D_ShaderOnRelease (void)
{
	int i;
...
{
	VID_RegisterHandlers (D_ShaderOnRelease, D_ShaderOnLost, D_ShaderOnReset);
}
...
void D_LoadShaderCompiler (void)
{
	...
	// error conditions
	if (!hInstCompiler) Sys_Error ("D_LoadShaderCompiler : LoadLibrary for d3dcompiler.dll failed");
	if (!QD3DCompile) Sys_Error ("D_LoadShaderCompiler : GetProcAddress for D3DCompile failed");
}
...
static HRESULT D_CompileShader (LPCVOID pSrcData, SIZE_T SrcDataSize, CONST D3D_SHADER_MACRO *pDefines, LPCSTR pEntrypoint, LPCSTR pTarget, ID3DBlob **ppCode)
{
	...
	if (errorBlob)
	{
		...
		VID_Printf (va ("Error compiling %s:\n%s\n", pEntrypoint, compileError));
...
static char *D_LoadShaderSource (int resourceID)
{
	...
	char *fullsource = (char *) Hunk_Alloc (LOAD_HUNK, includelength + shaderlength + 2);
