...
void R_ReadPointFile_f (void)
{
	...
	if (!f)
	{
		Con_Printf (PRINT_NORMAL, va ("couldn't open %s\n", name));
		return;
	}

	Con_Printf (PRINT_NORMAL, va ("Reading %s...\n", name));
	c = 0;
	...
	fclose (f);
	Con_Printf (PRINT_NORMAL, va ("%i points read\n", c));
}
...
void R_ParseParticleEffect2 (void)
{
	...
	for (i = 0; i < 3; i++) org[i] = MSG_ReadCoord ();
	for (i = 0; i < 3; i++) dmin[i] = MSG_ReadFloat ();
...
void R_ParseParticleEffect3 (void)
{
	...
	for (i = 0; i < 3; i++) org[i] = MSG_ReadCoord ();
	for (i = 0; i < 3; i++) box[i] = MSG_ReadByte ();
...
void R_UpdateParticles (void)
{
	...
	for (p = active_particles; p; p = p->next)
	{
		...
		if (p->type == pt_rain)
		{
			...
			p->org[0] += p->vel[0] * (frametime - 0.004);
			p->org[1] += p->vel[1] * (frametime - 0.004);
			p->org[2] += p->vel[2] * (frametime - 0.004);
		}
