...
void PF_error (void)
{
	...
	s = PF_VarString (0);
	Con_Printf (PRINT_NORMAL, va ("======SERVER ERROR in %s:\n%s\n", pr_strings + pr_xfunction->s_name, s));
...
void PF_objerror (void)
{
	...
	s = PF_VarString (0);
	Con_Printf (PRINT_NORMAL, va ("======OBJECT ERROR in %s:\n%s\n", pr_strings + pr_xfunction->s_name, s));
	ed = PROG_TO_EDICT (pr_global_struct->self);
	ED_Print (ed);
	ED_Free (ed);
...
void PF_setpuzzlemodel (void)
{
	...
	{
		//		PR_RunError ("no precache: %s\n", NewName);
		Con_Printf (PRINT_NORMAL, "**** NO PRECACHE FOR PUZZLE PIECE:");
...
void PF_sprint (void)
{
	...
	{
		Con_Printf (PRINT_NORMAL, "tried to sprint to a non-client\n");
		return;
	}
...
void PF_ambientsound (void)
{
	...
	if (!*check)
	{
		Con_Printf (PRINT_NORMAL, va ("no precache: %s\n", samp));
		return;
	}
...
void PF_break (void)
{
	...
	if (!DidIt)
	{
		...
		Con_Printf (PRINT_NORMAL, "break statement\n");
		DebugBreak ();
...
void PF_dprint (void)
{
	Con_Printf (PRINT_DEVELOPER, va ("%s", PF_VarString (0)));
}
...
void PF_aim (void)
{
	...
	Vector3Madf (end, dir, 2048, start);
	...
	for (i = 1; i < sv.num_edicts; i++, check = NEXT_EDICT (check))
	{
		...
		dist = Vector3Dot (dir, pr_global_struct->v_forward);
...
void PF_matchAngleToSlope (void)
{
	...
	mod = Vector3Dot (v_forward, old_right);
	...
	dot = Vector3Dot (v_forward, old_forward);
...
void PF_updateInfoPlaque (void)
{
	...
	else PR_RunError ("PF_updateInfoPlaque: bad objective index %u", idx);

	if (((mode & 1 /* FORCE_ON */) && (*use & check)) || ((mode & 2 /* FORCE_OFF */) && !(*use & check)))
		;
	else
	{
		*use ^= check;
...
builtin_t pr_builtin[] =
{
	...
	PF_break,									// voi () break														= #6
	PF_random,									// floa () random														= #7
	...
	PF_Spawn,									// entit () spawn														= #14
	PF_Remove,									// void(entity e) remove											= #15
	PF_traceline,								// float(vector v1, vector v2, float tryents) traceline	= #16
	PF_checkclient,							// entit () clientlist												= #17
