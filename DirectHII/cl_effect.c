...
// All changes need to be in SV_SendEffec (), SV_ParseEffec (),
// SV_SaveEffect (), SV_LoadEffect (), CL_ParseEffec ()
...
void SV_ParseEffect (sizebuf_t *sb)
{
	...
	case CE_ONFIRE:
		VectorCopy (G_VECTOR (OFS_PARM1), sv.Effects[index].Smoke.origin);
		VectorCopy (G_VECTOR (OFS_PARM2), sv.Effects[index].Smoke.velocity);
		sv.Effects[index].Smoke.framelength = HX_FRAME_TIME;
...
void CL_ParseEffect (void)
{
	...
	case CE_TELEPORTERPUFFS:
		...
		cl.Effects[index].Teleporter.framelength = HX_FRAME_TIME;
		dir = 0;
...
void CL_UpdateEffects (void)
{
	...
	for (index = 0; index < MAX_EFFECTS; index++)
	{
		...
		case CE_RAIN:
			...
			{
				R_RainEffect (org, org2, x_dir, y_dir, cl.Effects[index].Rain.color, cl.Effects[index].Rain.count);
		...
		case CE_SNOW:
			...
			distance = Vector3Normalize (snow_org);
			...
			{
				R_SnowEffect (org, org2, cl.Effects[index].Rain.flags, alldir, cl.Effects[index].Rain.count);
			...
			else
			{
				for (i = 0; i < cl.Effects[index].Chunk.numChunks; i++)
				{
					...
					case THINGTYPE_FLESH:
						if (moving) CL_RocketTrail (ent, 17);
					...
					case THINGTYPE_GLASS:
						break;
					case THINGTYPE_ICE:
						if (moving) CL_RocketTrail (ent, rt_ice);
					...
					case THINGTYPE_REDGLASS:
						break;
					case THINGTYPE_ACID:
						if (moving) CL_RocketTrail (ent, rt_acidball);
...
static int NewEffectEntity (void)
{
	...
	if (cl_numvisedicts == MAX_VISEDICTS) return -1;
	if (EffectEntityCount == MAX_EFFECT_ENTITIES) return -1;
