...
cvar_t midi_volume = {"midi_volume", "100", true};
...
void MIDI_Loop_f (void)
{
	...
	if (bLooped) Con_Printf (PRINT_NORMAL, "MIDI music will be looped\n");
	else Con_Printf (PRINT_NORMAL, "MIDI music will not be looped\n");
}

void MIDI_Volume_f (void)
{
	dwVolumePercent = (DWORD) (midi_volume.value * 65535) / 100;
...
BOOL MIDI_Init (void)
{
	...
	Cmd_AddCommand ("midi_resume", MIDI_Resume_f);
	Cmd_AddCommand ("midi_loop", MIDI_Loop_f);

	Cvar_RegisterVariable (&midi_volume, MIDI_Volume_f);
...
void MIDI_Play (char *Name)
{
	...
	{
		Con_Printf (PRINT_NORMAL, va ("Couldn't load midi file %s\n", Temp));
	}
	else
	{
		bFileOpen = TRUE;

		Con_Printf (PRINT_DEVELOPER, va ("Playing midi file %s\n", Temp));
	...
	case MOM_DONE:
		...
		if (bPlaying && nEmptyBuffers)
		{
			...
			if (nChkErr != CONVERTERR_NOERROR)
			{
				...
				{
					Con_Printf (PRINT_NORMAL, "MidiPro () conversion pass failed!");
					ConverterCleanup ();
					return;
				}
			}
