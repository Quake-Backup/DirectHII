...
entity_t	*CL_EntityNum (int num)
{
	if (num >= cl.num_entities)
	{
		...
		{
			cl_entities[cl.num_entities].lerpflags = LERP_RESETALL;
...
void CL_KeepaliveMessage (void)
{
	...
	// write out a nop
	Con_Printf (PRINT_NORMAL, "--> client to server keepalive\n");
...
void CL_ParseServerInfo (void)
{
	...
	if (i != PROTOCOL_VERSION && i != PROTOCOL_VERSION2)
	{
		Con_Printf (PRINT_NORMAL, va ("Server returned version %i, not %i or 0x%x", i, PROTOCOL_VERSION, PROTOCOL_VERSION2));
		return;
	}
	...
	{
		Con_Printf (PRINT_NORMAL, va ("Bad maxclients (%u) from server\n", cl.maxclients));
		return;
	}

	cl.scores = Hunk_Alloc (MAP_HUNK, cl.maxclients * sizeof (*cl.scores));
	...
	Con_Printf (PRINT_NORMAL, va ("%c%s\n", 2, str));
	...
	for (nummodels = 1;; nummodels++)
	{
		...
		{
			Con_Printf (PRINT_NORMAL, "Server sent too many model precaches\n");
			return;
		}
	...
	for (numsounds = 1;; numsounds++)
	{
		...
		{
			Con_Printf (PRINT_NORMAL, "Server sent too many sound precaches\n");
			return;
		}
	...
	for (i = 2; i < nummodels; i++)
	{
		...
		{
			Con_Printf (PRINT_NORMAL, va ("Model %s not found\n", model_precache[i]));
			return;
		}
	...
	// free any sounds that were not precached
	S_FreeUnusedSounds ();
	...
	puzzle_strings = (char *) FS_LoadFile (MAP_HUNK, "puzzles.txt");
...
void CL_ParseUpdate (int bits)
{
	...
	*set_ent = *ref_ent;
	...
	if (cl.Protocol == PROTOCOL_VERSION2)
	{
		for (i = 0; i < 3; i++)
		{
			if (bits & OriginBits[i])
				set_ent->origin[i] = ent->msg_origins[0][i] = MSG_ReadFloat ();
			else ent->msg_origins[0][i] = ref_ent->origin[i];

			if (bits & AnglesBits[i])
				set_ent->angles[i] = ent->msg_angles[0][i] = MSG_ReadFloat ();
			else ent->msg_angles[0][i] = ref_ent->angles[i];
		}
	}
	else
	{
		...
		{
			if (bits & OriginBits[i])
				set_ent->origin[i] = ent->msg_origins[0][i] = MSG_ReadCoord ();
			else ent->msg_origins[0][i] = ref_ent->origin[i];

			if (bits & AnglesBits[i])
				set_ent->angles[i] = ent->msg_angles[0][i] = MSG_ReadAngle ();
			else ent->msg_angles[0][i] = ref_ent->angles[i];
		}
	}
	...
	// setting ent->lerpfinish in both cases so we can just read that without any messy logic in the lerping code itself
	if (bits & U_LERPFINISH)
	{
		ent->lerpfinish = MSG_ReadFloat ();
		ent->lerpflags |= LERP_FINISH;
	}
	else
	{
		ent->lerpfinish = HX_FRAME_TIME;
		ent->lerpflags &= ~LERP_FINISH;
	}
...
{
	entity_t *ent = (entity_t *) Hunk_Alloc (MAP_HUNK, sizeof (entity_t));
...
#define SHOWNET(x) if(cl_shownet.value==2)Con_Printf (PRINT_NORMAL, va ("%3i:%s\n", msg_readcount-1, x));
...
void CL_ParseServerMessage (void)
{
	...
	{
		Con_Printf (PRINT_NORMAL, va ("Time: %2.2f Pck: %i ", realtime - lasttime, net_message.cursize));
		lasttime = realtime;
	}
	...
	while (1)
	{
		...
		{
			if (cl_shownet.value == 1)
				Con_Printf (PRINT_NORMAL, va ("Ent: %i (%i bytes)", EntityCount, EntitySize));
		...
		case svc_nop:
			//			Con_Printf (PRINT_NORMAL, "svc_nop\n");
			break;
		...
		case svc_version:
			...
			if (i != PROTOCOL_VERSION && i != PROTOCOL_VERSION2)
				Host_Error ("CL_ParseServerMessage: Server is protocol %i instead of %i or 0x%x\n", i, PROTOCOL_VERSION, PROTOCOL_VERSION2);
		...
		case svc_print:
			...
			if (intro_playing)
				...
				Con_Printf (PRINT_NORMAL, va ("%s", MSG_ReadString ()));
		...
		case svc_spawnbaseline:
			i = MSG_ReadShort ();
			// must use CL_EntityNu () to force cl.num_entities up
