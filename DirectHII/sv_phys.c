...
void SV_CheckAllEnts (void)
{
	...
	for (e = 1; e < sv.num_edicts; e++, check = NEXT_EDICT (check))
	{
		...
		if (SV_TestEntityPosition (check))
			Con_Printf (PRINT_NORMAL, "entity in invalid position\n");
	}
}
...
void SV_CheckVelocity (edict_t *ent)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		{
			Con_Printf (PRINT_DEVELOPER, va ("Got a NaN velocity on %s\n", pr_strings + ent->v.classname));
			ent->v.velocity[i] = 0;
		}

		if (IS_NAN (ent->v.origin[i]))
		{
			Con_Printf (PRINT_DEVELOPER, va ("Got a NaN origin on %s\n", pr_strings + ent->v.classname));
			ent->v.origin[i] = 0;
		}
...
qboolean SV_RunThink (edict_t *ent)
{
	...
	// sendinterval may be used for alias models other than movetype_step entities; i.e. it's also used for frame lerping
	if (!ent->free && ent->v.nextthink)
	{
		if (ent->v.nextthink - thinktime != HX_FRAME_TIME)
...
int ClipVelocity (vec3_t in, vec3_t normal, vec3_t out, float overbounce)
{
	...
	backoff = Vector3Dot (in, normal) * overbounce;
	...
	for (e = 1; e < sv.num_edicts; e++, check = NEXT_EDICT (check))
	{
		...
		for (t = 0; t < 13; t++)
		{
			...
			case 8:
				...
				//					amove_mag=Vector3Normalize(amove_norm);
				//					//Vector3Normalize(org);
		...
		if (block)
		{
			...
			for (i = 0; i < num_moved; i++)
			{
				...
				moved_edict[i]->v.angles[1] -= amove[1];
...
void SV_CheckStuck (edict_t *ent)
{
	...
	if (!SV_TestEntityPosition (ent))
	{
		Con_Printf (PRINT_DEVELOPER, "moved back Unstuck.\n");
		SV_LinkEdict (ent, true);
		return;
	}
	...
	for (z = 0; z < 18; z++)
		for (i = -1; i <= 1; i++)
			for (j = -1; j <= 1; j++)
			{
				...
				if (!SV_TestEntityPosition (ent))
				{
					Con_Printf (PRINT_DEVELOPER, "scooted Unstuck.\n");
					SV_LinkEdict (ent, true);
					return;
				}
			}
	...
	if (ent->v.oldorigin != ent->v.origin)
		Con_Printf (PRINT_DEVELOPER, "player is stuck.\n");
}
...
void SV_WallFriction (edict_t *ent, trace_t *trace)
{
	...
	d = Vector3Dot (trace->plane.normal, forward);
...
void SV_Physics_Client (edict_t *ent, int num)
{
	...
	case MOVETYPE_SWIM:
		...
		SV_FlyExtras (ent, sv.frametime, NULL);  // Hover & friction
		break;
	...
	case MOVETYPE_NOCLIP:
		...
		Vector3Madf (ent->v.origin, ent->v.velocity, sv.frametime, ent->v.origin);
		break;
...
void SV_Physics_Noclip (edict_t *ent)
{
	...
	Vector3Madf (ent->v.angles, ent->v.avelocity, sv.frametime, ent->v.angles);
	Vector3Madf (ent->v.origin, ent->v.velocity, sv.frametime, ent->v.origin);
...
void SV_Physics_Toss (edict_t *ent)
{
	...
	if (((int) ent->v.flags & FL_ONGROUND))
		...
		//@@
		if (Vector3Compare (ent->v.basevelocity, vec_origin))
			return;
	...
	// add gravity
	if (ent->v.movetype != MOVETYPE_FLY && ent->v.movetype != MOVETYPE_BOUNCEMISSILE && ent->v.movetype != MOVETYPE_FLYMISSILE && ent->v.movetype != MOVETYPE_SWIM)
		SV_AddGravity (ent);
...
void SV_Physics_Step (edict_t *ent)
{
	...
	if (!Vector3Compare (ent->v.velocity, vec_origin) || !Vector3Compare (ent->v.basevelocity, vec_origin))
	{
		ent->v.flags = (int) ent->v.flags & ~FL_ONGROUND;
...
void SV_Physics (double frametime)
{
	...
	srand ((unsigned int) (sv.time * 36.0));
	...
	for (i = 0; i < sv.num_edicts; i++, ent = NEXT_EDICT (ent))
	{
		...
		if (ent2 != sv.edicts)
		{
			originMoved = !Vector3Compare (ent->v.origin, oldOrigin);

			if (originMoved || !Vector3Compare (ent->v.angles, oldAngle))
			{
