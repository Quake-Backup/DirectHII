...
void SV_Edicts (char *Name)
{
	...
	if (!FH)
	{
		Con_Printf (PRINT_NORMAL, va ("Could not open %s\n", Name));
		return;
	}
...
void Sv_Edicts_f (void)
{
	...
	if (!sv.active)
	{
		Con_Printf (PRINT_NORMAL, "This command can only be executed on a server running a map\n");
		return;
	}
					...
					float attenuation)
{
	...
	{
		Con_Printf (PRINT_NORMAL, va ("SV_StartSound: %s not precached\n", sample));
		return;
	}
...
void SV_SendServerinfo (client_t *client)
{
	...
	MSG_WriteLong (&client->message, PROTOCOL_VERSION2);
...
void SV_ConnectClient (int clientnum)
{
	...
	Con_Printf (PRINT_DEVELOPER, va ("Client %s connected\n", client->netconnection->address));
...
void SV_AddToFatPVS (vec3_t org, mnode_t *node)
{
	...
	while (1)
	{
		...
		plane = node->plane;
		d = Vector3Dot (org, plane->normal) - plane->dist;
...
void SV_PrepareClientEntities (client_t *client, edict_t	*clent, sizebuf_t *msg)
{
	...
	// because bits are not necessarily consecutive
	int OriginBits[] = {U_ORIGIN0, U_ORIGIN1, U_ORIGIN2};
	int	AnglesBits[] = {U_ANGLES0, U_ANGLES1, U_ANGLES2};
...
skipA:;
		...
		if (!DoRemove)
		{
			...
			else if (ent->v.movetype == MOVETYPE_FLYMISSILE || ent->v.movetype == MOVETYPE_BOUNCEMISSILE || ent->v.movetype == MOVETYPE_BOUNCE)
			{
				if (!DoMissiles)
					IgnoreEnt = true;
			}
		...
		while (position < reference->count && e > reference->states[position].index)
			position++;
		...
		*set_ent = *ref_ent;
		...
		if (sv.Protocol == PROTOCOL_VERSION2)
		{
			...
			for (i = 0; i < 3; i++)
			{
				if (ent->v.origin[i] != ref_ent->origin[i])
				{
					bits |= OriginBits[i];
					set_ent->origin[i] = ent->v.origin[i];
				}

				if (ent->v.angles[i] != ref_ent->angles[i])
				{
					bits |= AnglesBits[i];
					set_ent->angles[i] = ent->v.angles[i];
				}
			}
		}
		else
		{
			for (i = 0; i < 3; i++)
			{
				if (QuantizedOrigin (ent->v.origin[i]) != QuantizedOrigin (ref_ent->origin[i]))
				{
					bits |= OriginBits[i];
					set_ent->origin[i] = ent->v.origin[i];
				}

				if (QuantizedAngles (ent->v.angles[i]) != QuantizedAngles (ref_ent->angles[i]))
				{
					bits |= AnglesBits[0];
					set_ent->angles[i] = ent->v.angles[i];
				}
		...
		if (ref_ent->skin != ent->v.skin || ref_ent->drawflags != ent->v.drawflags)
		{
			bits |= U_SKIN;
		...
		if (bits >= 65536) bits |= U_MOREBITS2;
		...
		if (sv.Protocol == PROTOCOL_VERSION2)
		{
			if (bits & U_ORIGIN0) MSG_WriteFloat (msg, ent->v.origin[0]);
			if (bits & U_ANGLES0) MSG_WriteFloat (msg, ent->v.angles[0]);
			if (bits & U_ORIGIN1) MSG_WriteFloat (msg, ent->v.origin[1]);
			if (bits & U_ANGLES1) MSG_WriteFloat (msg, ent->v.angles[1]);
			if (bits & U_ORIGIN2) MSG_WriteFloat (msg, ent->v.origin[2]);
			if (bits & U_ANGLES2) MSG_WriteFloat (msg, ent->v.angles[2]);
		}
		else
		{
			if (bits & U_ORIGIN0) MSG_WriteCoord (msg, ent->v.origin[0]);
			if (bits & U_ANGLES0) MSG_WriteAngle (msg, ent->v.angles[0]);
			if (bits & U_ORIGIN1) MSG_WriteCoord (msg, ent->v.origin[1]);
			if (bits & U_ANGLES1) MSG_WriteAngle (msg, ent->v.angles[1]);
			if (bits & U_ORIGIN2) MSG_WriteCoord (msg, ent->v.origin[2]);
			if (bits & U_ANGLES2) MSG_WriteAngle (msg, ent->v.angles[2]);
		}
		...
		// a byte is not enough resolution to encode the lower thinktimes used by h2
		if (bits & U_LERPFINISH) MSG_WriteFloat (msg, ent->v.nextthink - sv.time);
...
int SV_ModelIndex (char *name)
{
	...
	{
		Con_Printf (PRINT_NORMAL, va ("SV_ModelIndex: model %s not precached\n", name));
		return 0;
	}
...
#endif
{
	...
	Con_Printf (PRINT_DEVELOPER, va ("SpawnServer: %s\n", server));
	...
	// set the correct protocol version here
	sv.Protocol = PROTOCOL_VERSION2;
	...
	sv.states = Hunk_Alloc (MAP_HUNK, svs.maxclients * sizeof (client_state2_t));
	memset (sv.states, 0, svs.maxclients * sizeof (client_state2_t));

	sv.max_edicts = MAX_EDICTS;

	sv.edicts = Hunk_Alloc (MAP_HUNK, sv.max_edicts * pr_edict_size);
	...
	if (!sv.worldmodel)
	{
		Con_Printf (PRINT_NORMAL, va ("Couldn't spawn server %s\n", sv.modelname));
		sv.active = false;
	...
	Con_Printf (PRINT_DEVELOPER, "Server spawned.\n");
