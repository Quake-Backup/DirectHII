...
char *PR_ValueString (etype_t type, eval_t *val)
{
	...
	case ev_function:
		...
		sprintf (line, "% ()", pr_strings + f->s_name);
		break;
	case ev_field:
...
void ED_Print (edict_t *ed)
{
	...
	if (ed->free)
	{
		Con_Printf (PRINT_NORMAL, "FREE\n");
		return;
	}

	Con_Printf (PRINT_NORMAL, va ("\nEDICT %i:\n", NUM_FOR_EDICT (ed)));
	...
	for (i = 1; i < progs->numfielddefs; i++)
	{
		...
		Con_Printf (PRINT_NORMAL, va ("%s", name));
		l = strlen (name);
...
void ED_PrintEdicts (void)
{
	...
	Con_Printf (PRINT_NORMAL, va ("%i entities\n", sv.num_edicts));
...
void ED_PrintEdict_f (void)
{
	...
	if (i >= sv.num_edicts)
	{
		Con_Printf (PRINT_NORMAL, "Bad edict number\n");
		return;
	}
...
void ED_Count (void)
{
	...
	Con_Printf (PRINT_NORMAL, va ("num_edicts:%3i\n", sv.num_edicts));
	Con_Printf (PRINT_NORMAL, va ("active    :%3i\n", active));
	Con_Printf (PRINT_NORMAL, va ("view      :%3i\n", models));
	Con_Printf (PRINT_NORMAL, va ("touch     :%3i\n", solid));
	Con_Printf (PRINT_NORMAL, va ("step      :%3i\n", step));
}
...
void ED_ParseGlobals (char *data)
{
	...
	while (1)
	{
		...
		if (!key)
		{
			Con_Printf (PRINT_NORMAL, va ("'%s' is not a global\n", keyname));
			continue;
		}
...
char *ED_NewString (char *string)
{
	...
	l = strlen (string) + 1;
	new_string = Hunk_Alloc (MAP_HUNK, l);
	new_p = new_string; // preserve the pointer so we can return it
...
qboolean	ED_ParseEpair (void *base, ddef_t *key, char *s)
{
	...
	case ev_field:
		...
		if (!def)
		{
			Con_Printf (PRINT_NORMAL, va ("Can't find field %s\n", s));
			return false;
		}
	...
	case ev_function:
		...
		if (!func)
		{
			Con_Printf (PRINT_NORMAL, va ("Can't find function %s\n", s));
			return false;
		}
...
char *ED_ParseEdict (char *data, edict_t *ent)
{
	...
	while (1)
	{
		...
		if (!key)
		{
			Con_Printf (PRINT_NORMAL, va ("'%s' is not a field\n", keyname));
			continue;
		}
...
void ED_LoadFromFile (char *data)
{
	...
	while (1)
	{
		...
		if (!ent->v.classname)
		{
			Con_Printf (PRINT_NORMAL, "No classname for:\n");
			ED_Print (ent);
			ED_Free (ent);
			continue;
		}
		...
		if (!func)
		{
			Con_Printf (PRINT_NORMAL, "No spawn function for:\n");
			ED_Print (ent);
			ED_Free (ent);
			continue;
		}
	...
	Con_Printf (PRINT_DEVELOPER, va ("%i entities inhibited\n", inhibit));
}
...
void PR_LoadProgs (void)
{
	...
	progs = (dprograms_t *) FS_LoadFile (MAP_HUNK, finalprogname);

	if (!progs)
		Sys_Error ("PR_LoadProgs: couldn't load %s", finalprogname);

	Con_Printf (PRINT_DEVELOPER, va ("Programs occupy %iK.\n", com_filesize / 1024));
...
{
	// this is Portals only
	int i, count, start, Length;
	char NewLineChar;

	pr_global_info_strings = (char *) FS_LoadFile (GAME_HUNK, "infolist.txt");
	...
	pr_info_string_index = (int *) Hunk_Alloc (GAME_HUNK, (count + 1) * 4);
	...
	Con_Printf (PRINT_NORMAL, va ("Read in %d objectives\n", count));
}
...
void PR_LoadStrings (void)
{
	...
	pr_global_strings = (char *) FS_LoadFile (GAME_HUNK, "strings.txt");
	...
	pr_string_index = (int *) Hunk_Alloc (GAME_HUNK, (count + 1) * 4);
	...
	Con_Printf (PRINT_NORMAL, va ("Read in %d string lines\n", count));
}
...
int NUM_FOR_EDICT (edict_t *e)
{
	...
	if (b < 0 || b >= sv.num_edicts)
	{
		if (!RemoveBadReferences)
			Con_Printf (PRINT_DEVELOPER, va ("NUM_FOR_EDICT: bad pointer, Class: %s Field: %s, Index %d, Total %d", class_name, field_name, b, sv.num_edicts));
