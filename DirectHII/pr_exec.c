...
static void PrintCallHistory (void)
{
	...
	if (pr_depth == 0)
	{
		Con_Printf (PRINT_NORMAL, "<NO STACK>\n");
		return;
	}
	...
	for (i = pr_depth; i >= 0; i--)
	{
		...
		if (!f)
		{
			Con_Printf (PRINT_NORMAL, "<NO FUNCTION>\n");
		}
		else
		{
			Con_Printf (PRINT_NORMAL, va ("%12s : %s\n", pr_strings + f->s_file, pr_strings + f->s_name));
		}
	}
}
...
static void PrintStatement (dstatement_t *s)
{
	...
	{
		Con_Printf (PRINT_NORMAL, va ("%s ", pr_opnames[s->op]));
	...
	{
		Con_Printf (PRINT_NORMAL, va ("%sbranch %i", PR_GlobalString (s->a), s->b));
	}
	else if (s->op == OP_GOTO)
	{
		Con_Printf (PRINT_NORMAL, va ("branch %i", s->a));
	}
	else if ((unsigned) (s->op - OP_STORE_F) < 6)
	{
		Con_Printf (PRINT_NORMAL, va ("%s", PR_GlobalString (s->a)));
		Con_Printf (PRINT_NORMAL, va ("%s", PR_GlobalStringNoContents (s->b)));
	}
	else
	{
		if (s->a)
		...
		if (s->b)
		{
			Con_Printf (PRINT_NORMAL, va ("%s", PR_GlobalString (s->b)));
		}

		if (s->c)
		{
			Con_Printf (PRINT_NORMAL, va ("%s", PR_GlobalStringNoContents (s->c)));
		}
	}
