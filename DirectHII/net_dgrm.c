...
void NET_Printf (char *message)
{
	Con_Printf (PRINT_NORMAL, message);
}
...
int	Datagram_GetMessage (qsocket_t *sock)
{
	...
	while (1)
	{
		...
		if (length == -1)
		{
			Con_Printf (PRINT_NORMAL, "Read error\n");
			return -1;
		}
		...
		if (flags & NETFLAG_UNRELIABLE)
		{
			...
			{
				Con_Printf (PRINT_DEVELOPER, "Got a stale datagram\n");
				ret = 0;
				break;
			}
			...
			if (sequence != sock->unreliableReceiveSequence)
			{
				...
				Con_Printf (PRINT_DEVELOPER, va ("Dropped %u datagram(s)\n", count));
			}
		...
		if (flags & NETFLAG_ACK)
		{
			...
			{
				Con_Printf (PRINT_DEVELOPER, "Stale ACK received\n");
				continue;
			}
			...
			if (sequence == sock->ackSequence)
			{
				...
				if (sock->ackSequence != sock->sendSequence)
					Con_Printf (PRINT_DEVELOPER, "ack sequencing error\n");
			}
			else
			{
				Con_Printf (PRINT_DEVELOPER, "Duplicate ACK received\n");
				continue;
			}
...
{
	Con_Printf (PRINT_NORMAL, va ("canSend = %4u   \n", s->canSend));
	Con_Printf (PRINT_NORMAL, va ("sendSeq = %4u   ", s->sendSequence));
	Con_Printf (PRINT_NORMAL, va ("recvSeq = %4u   \n", s->receiveSequence));
...
void NET_Stats_f (void)
{
	...
	if (Cmd_Argc () == 1)
	{
		Con_Printf (PRINT_NORMAL, va ("unreliable messages sent   = %i\n", unreliableMessagesSent));
		Con_Printf (PRINT_NORMAL, va ("unreliable messages recv   = %i\n", unreliableMessagesReceived));
		Con_Printf (PRINT_NORMAL, va ("reliable messages sent     = %i\n", messagesSent));
		Con_Printf (PRINT_NORMAL, va ("reliable messages received = %i\n", messagesReceived));
		Con_Printf (PRINT_NORMAL, va ("packetsSent                = %i\n", packetsSent));
		Con_Printf (PRINT_NORMAL, va ("packetsReSent              = %i\n", packetsReSent));
		Con_Printf (PRINT_NORMAL, va ("packetsReceived            = %i\n", packetsReceived));
		Con_Printf (PRINT_NORMAL, va ("receivedDuplicateCount     = %i\n", receivedDuplicateCount));
		Con_Printf (PRINT_NORMAL, va ("shortPacketCount           = %i\n", shortPacketCount));
		Con_Printf (PRINT_NORMAL, va ("droppedDatagrams           = %i\n", droppedDatagrams));
	}
...
static void Test2_Poll (void)
{
	...
	Con_Printf (PRINT_NORMAL, va ("%-16.16s  %-16.16s\n", name, value));
...
Reschedule:
	SchedulePollProcedure (&test2PollProcedure, HX_FRAME_TIME);
	return;

Error:
	Con_Printf (PRINT_NORMAL, "Unexpected repsonse to Rule Info request\n");
Done:
...
static qsocket_t *_Datagram_Connect (char *host)
{
	...
	Con_Printf (PRINT_NORMAL, "trying...\n"); SCR_UpdateScreen ();
	start_time = net_time;
	...
	for (reps = 0; reps < 3; reps++)
	{
		...
		* ((int *) net_message.data) = BigLong (NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
		...
		Con_Printf (PRINT_NORMAL, "still trying...\n"); SCR_UpdateScreen ();
	...
	Con_Printf (PRINT_NORMAL, "Connection accepted\n");
