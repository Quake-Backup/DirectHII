...
static qboolean VCache_ActuallyReorderIndices (unsigned short *outIndices, const unsigned short *indices, int nTriangles, int nVertices)
{
	int i, j, sum, bestTriangle, bestScore, outPos, scanPos;
	int cache[VERTEX_CACHE_SIZE + 3];
	byte *numActiveTris = (byte *) Hunk_Alloc (LOAD_HUNK, sizeof (byte) * nVertices);
	...
	if (nVertices < VERTEX_CACHE_SIZE) return false;
	...
	offsets = (int *) Hunk_Alloc (LOAD_HUNK, sizeof (int) * nVertices);
	lastScore = (unsigned short *) Hunk_Alloc (LOAD_HUNK, sizeof (unsigned short) * nVertices);
	cacheTag = (char *) Hunk_Alloc (LOAD_HUNK, sizeof (char) * nVertices);

	//triangleAdded = (byte *) Hunk_Alloc (LOAD_HUNK, (nTriangles + 7) / 8);
	triangleAdded = (byte *) Hunk_Alloc (LOAD_HUNK, nTriangles);
	triangleScore = (unsigned short *) Hunk_Alloc (LOAD_HUNK, sizeof (unsigned short) * nTriangles);
	triangleIndices = (int *) Hunk_Alloc (LOAD_HUNK, sizeof (int) * 3 * nTriangles);
	...
	outTriangles = (int *) Hunk_Alloc (LOAD_HUNK, sizeof (int) * nTriangles);
	outPos = 0;
...
void VCache_ReorderIndices (unsigned short *outIndices, const unsigned short *indices, int nTriangles, int nVertices)
{
	...
	if (!VCache_ActuallyReorderIndices (outIndices, indices, nTriangles, nVertices))
	{
